...
class Genesis_Dambuster_Template {
   ...
   function prepare() {
		...
		if (is_singular() 
      && ($this->post_id = Genesis_Dambuster_Utils::get_post_id()) //get post/page id
		&& ($meta = Genesis_Dambuster_Utils::get_meta($this->post_id,  self::DAMBUSTER_METAKEY))
		&&	($this->post_options = Genesis_Dambuster_Options::validate_options($this->defaults, $meta))	      
		&& $this->post_options['enabled']) {  //we are tweaking this page
         $this->is_html5 = Genesis_Dambuster_Utils::is_html5();
         $this->is_landing = Genesis_Dambuster_Utils::is_landing_page();
   ...
   function remove_post_meta() {
	...
	function add_body_class( $classes ) {
      $classes[] = apply_filters('genesis_dambuster_body_class', 'gd-full-width');
      return $classes;
   }

	function enqueue_full_width_styles() {
	   $fullwidth = $this->is_html5 ? 'full-width.css' : 'full-width-xhtml.css' ;
		wp_enqueue_style('gd-full-width', plugins_url(  'styles/'.$fullwidth, dirname(__FILE__)), array(), '1.0');
      		
		if ($this->post_options['max_content_width']) {	
         wp_add_inline_style('gd-full-width', $this->inline_styles());
		}	
	...
	function inline_styles() { 
      ...
      $removebg = $this->post_options['remove_background'] ? 'body.gd-full-width { background-color: transparent; }' : '';
    	return <<< CSS
.gd-full-width .site-container .content .entry .inner { max-width: {$max_content_width}px; margin: 0 auto; padding: {$content_padding}; } 	
.gd-full-width .site-container .content .entry .inner.left { text-align:left; } 	
.gd-full-width .site-container .content .entry .inner.right { text-align:right; } 	
.gd-full-width .site-container .content .entry .inner.center { text-align:center; }
{$removebg}
CSS;
   }
