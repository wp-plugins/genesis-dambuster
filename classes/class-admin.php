...
abstract class Genesis_Dambuster_Admin {
   ...
   function enqueue_metabox_scripts() {
	...
	function form_field($id, $name, $label, $value, $type, $options = array(), $args = array(), $wrap = false) {
		...
		$label_args = (is_array($args) && array_key_exists('label_args', $args)) ? $args['label_args'] : false;
 		return Genesis_Dambuster_Utils::form_field($id, $name, $this->tooltips->tip($label, $label_args), $value, $type, $options, $args, $wrap);
 	}	
	...
	function print_text_field($fld, $value, $args = array()) {
 	...
 	function get_meta_form_data($metakey, $prefix, $values ) {
      $content = array();
		if (($post_id = Genesis_Dambuster_Utils::get_post_id())
		&& ($meta = Genesis_Dambuster_Utils::get_meta($post_id, $metakey)))
			$values = Genesis_Dambuster_Options::validate_options($values, $meta);	
   ...
   function get_newsfeeds() {
      return apply_filters('genesis_dambuster_newsfeeds', array(GENESIS_CLUB_NEWS, DIYWEBMASTERY_NEWS));
   }

 	function news_panel($post,$metabox){	
		Genesis_Dambuster_Feed_Widget::display_feeds($this->get_newsfeeds());
	}
	...
	function save_options($options_class, $settings_name, $trim_option_prefix = false) {
         ...
         $message= 'settings not found!';
  		...
  		$this->message = sprintf('<div class="%1$s"><p>%2$s %3$s</p></div>', $class, __($settings_name), __($message));
   ...
   function print_admin_form ($title, $referer = false, $keys = false, $enctype = false, $preamble = false) {
	...
	function display_metabox($tabs) {
      ...
      printf('<div class="genesis-dambuster-metabox"><ul class="genesis-dambuster-metabox-tabs">%1$s</ul><div class="metabox-content">%2$s</div><input type="hidden" id="tabselect" name="tabselect" value="%3$s" /></div>', $labels, $contents, $tab);
   }
